        ...
        if fields_to_add:
            ...
            with app.app_context():
                if 'likes' in fields_to_add:
                    db.engine.execute("ALTER TABLE poems ADD COLUMN likes INTEGER DEFAULT 0")
                    print("✓ 添加 likes 字段")
                
                if 'views' in fields_to_add:
                    db.engine.execute("ALTER TABLE poems ADD COLUMN views INTEGER DEFAULT 0")
                    print("✓ 添加 views 字段")
                
                if 'shares' in fields_to_add:
                    db.engine.execute("ALTER TABLE poems ADD COLUMN shares INTEGER DEFAULT 0")
                    print("✓ 添加 shares 字段")
                
                if 'tags' in fields_to_add:
                    db.engine.execute("ALTER TABLE poems ADD COLUMN tags TEXT")
                    print("✓ 添加 tags 字段")
                
                if 'difficulty_level' in fields_to_add:
                    db.engine.execute("ALTER TABLE poems ADD COLUMN difficulty_level VARCHAR(10) DEFAULT 'medium'")
                    print("✓ 添加 difficulty_level 字段")
                
                if 'theme_category' in fields_to_add:
                    db.engine.execute("ALTER TABLE poems ADD COLUMN theme_category VARCHAR(50)")
                    print("✓ 添加 theme_category 字段")
                
                if 'created_at' in fields_to_add:
                    db.engine.execute("ALTER TABLE poems ADD COLUMN created_at DATETIME DEFAULT CURRENT_TIMESTAMP")
